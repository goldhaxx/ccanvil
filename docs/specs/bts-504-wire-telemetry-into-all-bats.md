...
- [ ] **AC-3:** Given any `hub/tests/*.bats` file, when the injector classifies it, then it returns exactly one of `A|B|C|E|F|G|SKIP|UNCLASSIFIED` on stdout — multi-classification is structurally impossible because the 6 supported rows partition the 4-tuple boolean space disjointly. (Cat G added during Step 9 implementation when 3 teardown-only files surfaced; spec-up-on-reveal per `feedback_scope_up_on_live_api_reveal.md`.)
