...
* **AC-5 audit scope.** Likely targets to inspect (variables that COULD carry description-rich Linear JSON):
  ...
  * drift-watchdog: `$EXISTING`, `$DUP` — list-issues output, no description field by default; verify by reading the [linear-query.sh](<http://linear-query.sh>) `cmd_list_issues` body.
