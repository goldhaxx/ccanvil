...
Build a Claude Code-native scheduled agent that detects drift between the hub and registered downstream nodes and opens idempotent Linear issues per drift via the http substrate. Two-layer architecture: deterministic detection via a new `ccanvil-sync.sh drift-watchdog-list` subcommand; stochastic interpretation via a new `drift-analyst` sub-agent that reads drift JSON + recent git log + roadmap context and produces a thoughtful issue body. Hub-only pilot — no distribution to downstream nodes. The schedule trigger is **macOS launchd** (chosen after pre-flight smoke-tests revealed Claude Code's `CronCreate` is session-bound and auto-expires after 7 days — wrong shape for a weekly recurring job). The skill prints the exact `.plist` the operator should drop into `~/Library/LaunchAgents/`; loading the agent stays an explicit operator action.
...
- [ ] **AC-6:** Pre-flight verification subcommand `ccanvil-sync.sh drift-watchdog-preflight` emits JSON `{claude_p_available, linear_query_works}` — each `true|false`. `claude_p_available` checks `command -v claude` exit-0 (binary is on PATH); `linear_query_works` calls `linear-query.sh viewer` and asserts exit-0 + valid JSON output. Live-validation gate (BTS-171): both checks were validated against the live system before this spec was activated — the `-p` mode + linear-query.sh path ran end-to-end on 2026-04-26. Test mocks the underlying commands.
...
- [ ] **AC-9:** Schedule-entry surfacing — running `bash .ccanvil/scripts/ccanvil-sync.sh drift-watchdog-launchd-print` emits a complete macOS launchd `.plist` document on stdout, valid XML, with `Label=com.ccanvil.drift-watchdog`, `StartCalendarInterval` set to Monday 09:13 (Weekday=1 Hour=9 Minute=13 — non-:00 jitter per scheduling discipline), and `ProgramArguments` invoking `claude -p "/drift-watchdog"` against the hub repo. The skill's exit message references this subcommand and instructs the operator to write the output to `~/Library/LaunchAgents/com.ccanvil.drift-watchdog.plist` and `launchctl load` it. Test asserts the output contains the Label, the calendar fields, and the `claude -p` invocation; asserts the XML parses via `xmllint --noout`.
...
| File | Change |
|------|--------|
| `.ccanvil/scripts/ccanvil-sync.sh` | Modified (add `drift-watchdog-list`, `drift-watchdog-preflight`, `drift-watchdog-launchd-print` subcommands) |
...
- Auto-loading the launchd plist (`launchctl load ~/Library/LaunchAgents/...`). Operator does this manually after running `drift-watchdog-launchd-print > ~/Library/LaunchAgents/com.ccanvil.drift-watchdog.plist`.
- Linux/Windows scheduling. macOS launchd only for the pilot — Linux nodes would use systemd timers, but no Linux node currently runs the hub.
...
- launchd `.plist` template should use `StartCalendarInterval` (not `StartInterval`) so the schedule is calendar-anchored. Pick `Weekday=1 Hour=9 Minute=13` (Monday 09:13) — non-:00 jitter avoids fleet-coincidence per the CronCreate scheduling discipline. Include `RunAtLoad: false` (don't fire on `launchctl load`) and `WorkingDirectory` pointing at the hub repo so `claude -p` runs in the right context.
