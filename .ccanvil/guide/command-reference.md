...
| `ccanvil-sync.sh drift-watchdog-launchd-print` | Print a complete macOS launchd `.plist` to stdout. Label `com.ccanvil.drift-watchdog`, `StartCalendarInterval` Monday 09:13 (non-:00 jitter per scheduling discipline), `ProgramArguments` invoke `claude -p "/drift-watchdog" --max-budget-usd 0.50` from the hub repo. Operator workflow: `bash .ccanvil/scripts/ccanvil-sync.sh drift-watchdog-launchd-print > ~/Library/LaunchAgents/com.ccanvil.drift-watchdog.plist && launchctl load ~/Library/LaunchAgents/com.ccanvil.drift-watchdog.plist`. |
