...
Claude Code supports running multiple agents in parallel via git worktrees (`claude --worktree` or `-w`). The preset is compatible with this workflow.
...
| Hub sync | Don't run `/ccanvil-pull` from multiple worktrees simultaneously — lockfile mutations will conflict. |
