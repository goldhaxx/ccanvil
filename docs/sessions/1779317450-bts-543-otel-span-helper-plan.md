# Implementation Plan: Generic [otel-span.sh](<http://otel-span.sh>) helper library
...
### Step 2: [otel-span.sh](<http://otel-span.sh>) scaffold + ID generation (TDD)
...
### Step 8: Refactor [bats-report.sh](<http://bats-report.sh>) suite-root span (behavior-preserving)
...
* **Verify:** `module-manifest.sh validate --json` — status ok, [otel-span.sh](<http://otel-span.sh>) covered.
