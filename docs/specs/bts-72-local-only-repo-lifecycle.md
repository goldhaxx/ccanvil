...
- [ ] **AC-5:** `cmd_land` on a local-only repo (post-merge state: feature already merged into main, on main) detects the type, skips the `git fetch origin` + `git reset --hard origin/main` block, and still emits the AUTO-CLOSE marker for the recovered branch's Linear spec (work-ref tracking unchanged). Output contains `Already on main. No remote configured.` AND the AUTO-CLOSE JSON marker.
