...
- [ ] **AC-4:** `.claude/ccanvil.local.json:integrations.providers.linear.state_ids.done` is populated with the Blocktech "Done" UUID `bc6aa160-258d-4eae-b3b5-a2575732a188` as part of this ship. Verified by smoke test (not bats), because `ccanvil.local.json` is per-node and gitignored — downstream nodes verify via AC-5 (unconfigured-role loud error) when they invoke `ticket.transition <id> done` before migrating.
