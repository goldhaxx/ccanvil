...
| `permissions-audit.sh promote-review [--settings-dir DIR] [--text]` | BTS-144: list `settings.local.json` entries not in `settings.json` and classify each (DELETE / TRIAGE) for systematic review. Deterministic rules: redundant (covered by broader `Bash(<word>:*)` wildcard) → DELETE, `preset/` dead-path → DELETE, env-prefix bypass with broadly-allowed underlying verb (e.g., `Bash(ALLOW_OUTSIDE_WORKSPACE=1 bash ...)` when `Bash(bash:*)` exists in main) → DELETE, otherwise → TRIAGE. JSON output: `{candidates: [{permission, source, recommendation, reason}], counts: {delete, promote, triage, total}}`. `counts.promote` is always 0 in this version (PROMOTE reserved for future `--apply` flow). Exit 0 always — read-only review tooling. |
