...
- [ ] **AC-6:** Error: when [ccanvil-sync.sh](<http://ccanvil-sync.sh>) pull-plan exits non-zero (e.g., hub missing on disk, malformed lock), surface the wrapper's stderr verbatim under a `WRAPPER ERROR:` prefix and exit non-zero.
