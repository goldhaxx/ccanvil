...
| `ci-status-check.sh` | PostToolUse (Bash) | Yes, **on a confirmed failed run only** | After a `git push`, if the repo has GitHub Actions (`.github/workflows/*.y{a,}ml`), `gh` is authenticated, and `origin` is a github.com remote, polls `gh run list` (bounded by `CI_STATUS_CHECK_BUDGET` seconds, default 20) for the run(s) whose `headSha` matches the pushed HEAD. Exit 2 with a red-build summary (workflow + URL per failed run) so Claude fixes it in-session instead of the user learning via a failure email; exit 0 (quiet stderr note) on all-passed or still-running. **Self-gating no-op** — costs nothing on nodes with no CI / no `gh` / a non-GitHub remote. Knobs: `CI_STATUS_CHECK_BUDGET`, `CI_STATUS_CHECK_INTERVAL`, `CI_STATUS_CHECK_DISABLE`. |
