...
- [ ] **AC-3 (bijection guard):** Given the set of tier-0 rules and the set of sidecars, when `validate` runs, then every rule has exactly one sidecar and every `*.manifest.yaml` maps to an existing rule — any orphan sidecar or sidecar-less tier-0 rule is block-shape drift (exit 2).
