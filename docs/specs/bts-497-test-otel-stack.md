...
- **Pattern (implementation revised):** the original plan was to use `otel-cli span background` for a unix-socket span server with ~1-3 ms per-test emission. Discovered mid-implementation that the verb creates ONE long-running span you add EVENTS to, not a multi-span emission daemon. Final implementation uses direct per-test `otel-cli span --endpoint <url> --protocol http/protobuf` at ~5-15 ms per call — still well inside the AC-7 50 ms p95 budget for 2,338 tests (~12-35 s suite-wall cost). No background process means no kill/wait/teardown — terminating by construction. Reference: `docs/research/test-performance-research.md` §10.4.
...
- **Performance budget (AC-7):** the BTS-281 fixture-cache pattern handles validate cost; OTel emission adds ~5-15 ms per test via direct OTLP HTTP (revised from initial ~1-3 ms socket projection — see Pattern note above). Budget: 50 ms p95 marginal cost across the suite vs. pre-instrumentation baseline (current p50 = 83 ms, p95 = 1075 ms — telemetry must not move these visibly). Measurement deferred to BTS-504's full rollout (Phase D narrowing leaves only 10 files instrumented; full-suite delta meaningful only post-rollout).
