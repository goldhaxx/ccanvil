...
**The pattern this enables.** Every false-alert surface in ccanvil (shape-gate narrative cascade is the live example) has the same shape: a guard fires on a string that *looks* like the dangerous pattern but is *known-safe*. The right answer is never "lower the threshold"; it's "declare the known-safe shape in an SSOT both the guard and the author reference." Phase B unifies that pattern across guards. (Historically the workspace-fence false-alert family was a sibling — retired in BTS-602 when the cost-benefit on guard-workspace.sh inverted; the fence carried seven carve-outs and four dedicated bats files maintaining them, and every fire misclassified normal work.)
