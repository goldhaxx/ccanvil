...
- [ ] **AC-4 (definition forms):** Both the POSIX `name() {` form and the `function name {` / `function name() {` form are recognized as function-definition boundaries among added lines and update the attribution context.
- [ ] **AC-5 (no regression on same-function edits):** Given a diff that adds a marker inside an existing function's body with **no** function definition among the added lines, attribution is unchanged (the marker maps to the xfuncname function). The four existing `module-manifest-diff-vs-manifest.bats` cases continue to pass unmodified.
...
| File | Change |
|------|--------|
| `.ccanvil/scripts/module-manifest.sh` | Modified — `_diff_files_added` awk: track current-fn context across added lines; update on a function-definition line before assigning per-line `ctx`. Add a `contract:` line to `cmd_diff_vs_manifest`'s manifest documenting the behavior. |
...
- **Seam:** the awk in `_diff_files_added` (`module-manifest.sh` ~1632). Today `added_ctx[count]=hunk_ctx` is constant for the whole hunk. Introduce `current_fn` (init `= hunk_ctx` at each `@@`); when an added line matches `^[a-zA-Z_][a-zA-Z0-9_]*\(\)` **or** `^function[[:space:]]+[a-zA-Z_][a-zA-Z0-9_]*`, set `current_fn` to that line *before* `added_ctx[count]=current_fn`. `_diff_ctx_to_primitive_id` already parses the `name()` prefix, so feeding it the def line needs no caller-side change.
