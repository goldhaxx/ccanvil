...
- [ ] **AC-9:** Determining the just-merged feature-id on main is deterministic: parse the last commit on main matching `^feat(<id>):` where `<id>` maps to a `Complete`-status spec in `docs/specs/`. If no match (e.g. non-feature merge, or manual merge without convention), skip auto-close and log: `auto-close: no feature-id detected in last merge commit — skipping`.
