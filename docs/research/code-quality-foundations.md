...
> Status: evidence anchor for `.claude/rules/code-quality.md`
> Origin: BTS-666 (rule body atomization — the tier-0 rule keeps the directives; the catalog lives here)

The `code-quality` rule is a tier-0 always-loaded directive. Per the rule-content-tiering model (`docs/research/rule-content-tiering.md`), the per-category catalog below is reference material — accessed on demand, not on every turn — so it lives in this anchor rather than inline in the rule.
